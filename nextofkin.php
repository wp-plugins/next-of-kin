...
update_option('tz_nextofkin_ver', '1.0');
...
global $current_user;																	//but only if
	if (get_option('tz_nextofkin_active_'.$current_user->user_login) == true) {				//the plugin is activated in own menu per current user (not just plugins menu)
		if ( (time() - get_option('tz_nextofkin_ll_'.$current_user->user_login)) > 43200 ) {	//and writes the current time as last login time if 12 hours passed since last write
	...
	if ((time() - get_option('tz_nextofkin_lastiteration')) > 7200) { // The following is a bit heavy, so we'll only do it once every two hours
		...
		if (is_array($tz_nok_users)) {
			$tz_users = 0;
			while ($tz_users < count($tz_nok_users)) {
				if ($tz_nok_users[$tz_users] != '') {
				$cur_options = get_option('tz_nextofkin_options_'.$tz_nok_users[$tz_users]);
				$cur_optionsm = get_option('tz_nextofkin_optionsm_'.$tz_nok_users[$tz_users]);
				
				if ((time() - get_option('tz_nextofkin_ll_'.$tz_nok_users[$tz_users])) > ($cur_options['interval1'] * $weeks)) { // After interval 1
					if (get_option('tz_nextofkin_step1_'.$tz_nok_users[$tz_users]) != true) {
						...
						update_option('tz_nextofkin_step1_'.$tz_nok_users[$tz_users], true);
					}
				}
				
				if ((time() - get_option('tz_nextofkin_ll_'.$tz_nok_users[$tz_users])) > (($cur_options['interval1'] + $cur_options['interval2']) * $weeks)) { // After interval 2
					if (get_option('tz_nextofkin_step2_'.$tz_nok_users[$tz_users]) != true) {
						...
						update_option('tz_nextofkin_step2_'.$tz_nok_users[$tz_users], true);
					}
				}
				
				if ((time() - get_option('tz_nextofkin_ll_'.$tz_nok_users[$tz_users])) > (($cur_options['interval1'] + $cur_options['interval2'] + $cur_options['interval3']) * $weeks)) { // After interval 3 (Death)
					if (get_option('tz_nextofkin_step3_'.$tz_nok_users[$tz_users]) != true) {
						...
						update_option('tz_nextofkin_step3_'.$tz_nok_users[$tz_users], true);
					}
				}
...
_e('Options updated succesfully.', 'nextofkin'); 
    ...
    ?></strong></p></div><?php 
				...
				echo '
		<p>Time since last loaded page: '.time().' - '.get_option('tz_nextofkin_ll_'.$current_user->user_login).' = '.(time() - get_option('tz_nextofkin_ll_'.$current_user->user_login)).' Seconds.</p>
		<p>Current user: '.$current_user->user_login.'</p>
		...
		if (isset($_POST['tz_nok_update'])) {
			if ($_POST['tz_nok_act'] == 'on') { // If plugin was just enabled (through options menu, not activated in plugins page)
				...
				if ( !is_array($tz_nok_users) ) {
					$tz_nok_users = array('gotcha');
					echo 'setting array';
					}
					else {
					
						if (!in_array($current_user->user_login, $tz_nok_users)) {
						$tz_nok_users[count($tz_nok_users)] = $current_user->user_login;
							}
						}
			}
				else { 
...
if (get_option('tz_nextofkin_active_'.$current_user->user_login) == true) { // Most of what comes next only loads if plugin is enabled for the logged in user.
		...
		if (isset($_POST['tz_nok_update'])) { // If hit "update options" button
                                ...
                                <table class="optiontable">
                                        ...
                                        <tr valign="top">
                                                ...
                                                <td>
														...
														<option '.($options['interval1']=="1" ? "selected='selected'" : '').' value="1">1</option>
														<option '.($options['interval1']=="2" ? "selected='selected'" : '').' value="2">2</option>
														<option '.($options['interval1']=="3" ? "selected='selected'" : '').' value="3">3</option>
														<option '.($options['interval1']=="4" ? "selected='selected'" : '').' value="4">4</option>
														<option '.($options['interval1']=="5" ? "selected='selected'" : '').' value="5">5</option>
														<option '.($options['interval1']=="6" ? "selected='selected'" : '').' value="6">6</option>
														<option '.($options['interval1']=="7" ? "selected='selected'" : '').' value="7">7</option>
														<option '.($options['interval1']=="8" ? "selected='selected'" : '').' value="8">8</option>
														<option '.($options['interval1']=="9" ? "selected='selected'" : '').' value="9">9</option>
														<option '.($options['interval1']=="10" ? "selected='selected'" : '').' value="10">10</option>
														</select> '.__('Weeks', 'nextofkin').'
														</td>
                                        </tr>
                                        <tr>
                                              <th scope="row">'.__('Interval 2: (time between first warning mail to seconds warning mail)', 'nextofkin').'</th>
                                                <td>
														...
														<option '.($options['interval2']=="1" ? "selected='selected'" : '').' value="1">1</option>
														<option '.($options['interval2']=="2" ? "selected='selected'" : '').' value="2">2</option>
														<option '.($options['interval2']=="3" ? "selected='selected'" : '').' value="3">3</option>
														<option '.($options['interval2']=="4" ? "selected='selected'" : '').' value="4">4</option>
														<option '.($options['interval2']=="5" ? "selected='selected'" : '').' value="5">5</option>
														<option '.($options['interval2']=="6" ? "selected='selected'" : '').' value="6">6</option>
														<option '.($options['interval2']=="7" ? "selected='selected'" : '').' value="7">7</option>
														<option '.($options['interval2']=="8" ? "selected='selected'" : '').' value="8">8</option>
														<option '.($options['interval2']=="9" ? "selected='selected'" : '').' value="9">9</option>
														<option '.($options['interval2']=="10" ? "selected='selected'" : '').' value="10">10</option>
														</select> '.__('Weeks', 'nextofkin').'
                                                </td>
                                        </tr>
										<tr>
                                                ...
                                                <td>
														...
														<option '.($options['interval3']=="1" ? "selected='selected'" : '').' value="1">1</option>
														<option '.($options['interval3']=="2" ? "selected='selected'" : '').' value="2">2</option>
														<option '.($options['interval3']=="3" ? "selected='selected'" : '').' value="3">3</option>
														<option '.($options['interval3']=="4" ? "selected='selected'" : '').' value="4">4</option>
														<option '.($options['interval3']=="5" ? "selected='selected'" : '').' value="5">5</option>
														<option '.($options['interval3']=="6" ? "selected='selected'" : '').' value="6">6</option>
														<option '.($options['interval3']=="7" ? "selected='selected'" : '').' value="7">7</option>
														<option '.($options['interval3']=="8" ? "selected='selected'" : '').' value="8">8</option>
														<option '.($options['interval3']=="9" ? "selected='selected'" : '').' value="9">9</option>
														<option '.($options['interval3']=="10" ? "selected='selected'" : '').' value="10">10</option>
														</select> '.__('Weeks', 'nextofkin').'
                                                </td>
                                        </tr>										
                                        </tbody>
